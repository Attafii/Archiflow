#ifndef CONTRACTAIAASSISTANTDIALOG_H
#define CONTRACTAIAASSISTANTDIALOG_H
...
class ContractChatBubble : public QFrame
{
    Q_OBJECT
...
public:
    ...
    explicit ContractChatBubble(Type type, const QString &message, QWidget *parent = nullptr);
...
class ContractTypingIndicator : public QWidget
{
    Q_OBJECT

public:
    explicit ContractTypingIndicator(QWidget *parent = nullptr);
...
class ContractAIAssistantDialog : public QDialog
{
    Q_OBJECT

public:
    explicit ContractAIAssistantDialog(QWidget *parent = nullptr);
    ~ContractAIAssistantDialog();
    
    void setGroqClient(GroqClient *client);
    void setDatabaseService(DatabaseService *dbService);
    void setContractDatabaseManager(ContractDatabaseManager *dbManager);
    void setContractContext(const QJsonObject &context);
...
private:
    ...
    void addMessage(ContractChatBubble::Type type, const QString &message);
    void scrollToBottom();
    ...
    ContractTypingIndicator *m_typingIndicator;
    QProgressBar *m_progressBar;
    
    // Data & Logic
    GroqClient *m_groqClient;
    DatabaseService *m_databaseService;
    ContractDatabaseManager *m_contractDbManager;
    QList<ChatMessage> m_chatHistory;
    QJsonObject m_contractContext;
...
#endif // CONTRACTAIAASSISTANTDIALOG_H
